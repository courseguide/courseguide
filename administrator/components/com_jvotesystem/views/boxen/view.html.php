<?php
/**
 * @package Component jVoteSystem for Joomla! 1.5 - 2.5
 * @projectsite www.joomess.de/projects/jvotesystem
 * @authors Johannes Meßmer, Andreas Fischer
 * @copyright (C) 2010 - 2012 Johannes Meßmer
 * @license GNU/GPLv3 http://www.gnu.org/licenses/gpl-3.0.html
**/

//-- No direct access
defined('_JEXEC') or die('=;)');

jimport( 'joomla.application.component.view');

/**
 * HTML View class for the jVoteSystem Component
 *
 * @package    jVoteSystem
 * @subpackage Views
 */

class jVoteSystemViewBoxen extends JView
{
    /**
     * jVoteSystemList view display method
     * @return void
     **/
    function display($tpl = null)
    {
		$this->charts =& VBCharts::getInstance();
		$this->general =& VBGeneral::getInstance();
		//Daten laden
		$filter = $this->get('Filter');
		$items = $this->get('Data');
		$pagination = $this->get('Pagination');
		
		//initialise variables
		$document	= & JFactory::getDocument();
		
		//Toolbar
	        JToolBarHelper::addNewX();
	        JToolBarHelper::editListX();
		JToolBarHelper::divider();
			JToolBarHelper::publishList();
			JToolBarHelper::unpublishList();
	        JToolBarHelper::deleteList();
		JToolBarHelper::divider();
			//Reset
			JToolBarHelper::custom('resetVotes', 'trash', 'trash', JText::_("JVS_MENU_RESET_VOTES"), true);
			JToolBarHelper::custom('resetColors', 'trash', 'trash', JText::_("JVS_MENU_RESET_COLORS"), true);
		JToolBarHelper::divider();
			//Tools
			JToolBarHelper::custom('showPreview', 'preview', 'preview', JText::_("JVS_MENU_PREVIEW"), true);
			JToolBarHelper::custom('exportData', 'export', 'export', JText::_("JVS_MENU_EXPORT"), true);
		JToolBarHelper::divider();
			if(VBAccess::getInstance()->isUserAllowedToConfig())
				JToolBarHelper::preferences('com_jvotesystem', 500);
		
		//Load js file for tooltips
		$this->lib =& JoomessLibrary::getInstance();
		$this->lib->plugin("zebra_dialog");
		$this->lib->js('components/com_jvotesystem/assets/js/jvotesystem.js');
		
        //Daten übergeben
		$this->assignRef('items', $items);
		$this->assignRef('filter', $filter);
		$this->assignRef('pagination', $pagination);

        parent::display($tpl);
    }//function

}//class
