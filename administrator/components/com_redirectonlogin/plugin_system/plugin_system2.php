...
class plgSystemRedirectonlogin extends JPlugin{		
	...
	function onAfterRender(){	
		...
		if($app->isSite()){	
			...
			if(
			(strpos($buffer, JText::_('JERROR_ALERTNOAUTHOR')) || strpos($buffer, JText::_('JEV_LOGIN_TO_VIEWEVENT'))) && 
			...
			(strpos($buffer, '<dd class="error message">') || strpos($buffer, '<dd class="message message">')) && 
			strpos($buffer, '<dl id="system-message">')
			){						
				$uri = JFactory::getURI();
				$return = $uri->toString();
				if(strpos($buffer, JText::_('JEV_LOGIN_TO_VIEWEVENT'))){
