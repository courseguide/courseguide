...
class JMThumb
{
		...
		function create($data)
        {
                ...
                if (file_exists($data)) {
                        ...
                        $this->fullName = $data;
			                if ( substr(strtolower($this->fullName),-3) == 'gif' ) {
			                        $this->img = @ImageCreateFromGif($data);
			                } elseif ( substr(strtolower($this->fullName),-3) == 'jpg' ) {
			                        $this->img = @ImageCreateFromJPEG($data);		
			                } elseif ( substr(strtolower($this->fullName),-3) == 'png' ) {
        ...
        */
        function cubecut($cutX,$cutY) {
        	if ($this->img) {
        		...
        		if ($this->resize($newX,$newY)) {
        			return $this->autocut($cutX,$cutY, 5);
        		} else {
        			return false;
        		}
        	} else {
        ...
        function output($sendHeader = true,$destroy = true)
        {
                if ($this->img) {
                                ...
                                return ImageJPEG($this->img,"",$this->quality);
